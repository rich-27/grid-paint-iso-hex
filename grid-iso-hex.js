 ...
 */
 function getIsoHexCellPoints(col, row, sideLength) {
	...
	return [
		{x: x+halfWidth, y: y},
		{x: x, y: y+dy},
		{x: x, y: y+dy+sideLength},
		{x: x+halfWidth, y: y+2*dy+sideLength},
		{x: x+2*halfWidth, y: y+dy+sideLength},
		{x: x+2*halfWidth, y: y+dy},
		{x: x+halfWidth, y: y+dy+sideLength/2}
	]
}
...
function GridIsoHex() {
	...
	this.paintGrid=function(paper) {
		...
		for (x=halfWidth; x<paper.width; x+=halfWidth*2) {
		    this._createGridLine(
		    	paper,
		    	["M",x,2*dy+sideLength,"V",paper.height],
		    	stroke_dash_array);
		}
	...
	this.pointToCell=function(x,y) {
		...
		if (triangleCell.col % 2 == 0) {
			...
			if (triada % 2 == 0) {
				return {col: Math.floor(triangleCell.col / 2), row:  triada}
			} else {
				return {col: Math.floor(triangleCell.col / 2) - 1, row:  triada}
			}
		} else {
			var triada=Math.floor(triangleCell.row/3);
			if (triada % 2 == 0) {
	...
	this.shapes={
		// "empty": new GridIsoHex_ShapeEmpty(this),
